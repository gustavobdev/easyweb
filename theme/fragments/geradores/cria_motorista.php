...
foreach ($data as $key => $value) {
    if (empty($value) or is_null($value)) {
        ...
        $router->redirect("console/cria/motorista");
        die();
    }
}
...
$email = SanitizeEmail($data["email"], "console/cria/motorista");
$phone = phoneValidate($data["phone1"], "console/cria/motorista");
...
if ($motorista->save() != false) {
    ...
    if (!$mail->send()) {
        ...
        $router->redirect("/console/cria/motoristas");
    } else {
